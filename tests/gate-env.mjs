...
section('首頁旅團閘（Vercel 環境變數登記）');
{
  ...
  /* ★ 2026-09-24：venture82.mode.v2（真實／示範）已經隨示範模式一齊拆走，
...
/* ============================================================
   ...
   ------------------------------------------------------------
   ★ 2026-09-24：呢度以前係「示範模式唔會自動記住」。示範（MOCK）拆走咗，
...
/* ============================================================
   ⑤ 入咗旅團之後：側邊欄唔需要再「選擇旅團」
   ------------------------------------------------------------
   ★ 2026-09-24 團長：「進入旅團後,旁邊選單第二行『選擇旅團』沒有用，
     都進入了還選什麼？只須要下方登出」
...
section('入咗旅團：冇「選擇旅團」掣，但登出喺度');
{
  ...
  ok('登入頁仍有免登入讀寫測試（共用同一個後端自測）',
    /id=\"btnLoginReadWrite\"/.test(mainSrc) && /remoteApi\.testReadWrite\(\)/.test(mainSrc));
...
console.log(`\n──────── 旅團登記／登出清理測試結果：${pass} 通過 / ${fail} 失敗（${ms} ms）────────`);
